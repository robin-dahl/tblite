...
subroutine test_dipole_ss(error)
   ...
   call nativeint%dipole_cgto(cgtoi, cgtoj, 1, 2, r2, vec, &
      & 100.0_wp, overlap, dipolej)

   vec(:) = -vec

   call nativeint%dipole_cgto(cgtoj, cgtoi, 2, 1, r2, vec, &
...
subroutine test_dipole_grad_ss(error)
   ...
   call nativeint%multipole_grad_cgto(cgtoj, cgtoi, 1, 2, r2, &
   ...
   do i = 1, 3
      vec(i) = vec(i) + step
      r2 = sum(vec**2)
      call nativeint%multipole_cgto(cgtoj, cgtoi, 1, 2, r2, &
