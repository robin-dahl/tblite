...
!> @file tblite/integral/native/native.f90
!> Provides native evaluation of Gaussian integrals.

!> Native implementation of Gaussian integrals.
module tblite_integral_native
   ...
   use tblite_integral_handler, only : integral_handler, &
      & maxl, maxl2, msao, mlao, smap, lmap, sdim, lx
   use tblite_integral_native_integrals, only: overlap_1d, dipole_3d, multipole_3d, &
      & multipole_grad_3d, shift_operator
...
pure subroutine multipole_cgto(self, cgtoj, cgtoi, jsh, ish, r2, vec, intcut, &
      & overlap, dpint, qpint)
   !> Native integral evaluator
   class(native_integral_type), intent(in) :: self
   !> Description of contracted Gaussian function on center j
...
pure subroutine multipole_grad_cgto(self, cgtoj, cgtoi, jsh, ish, r2, vec, intcut, &
...
subroutine get_dipole_integrals_lat(mol, trans, cutoff, bas, overlap, dpint)
   ...
   !$omp shared(mol, bas, trans, cutoff2, overlap, dpint, nativeint) &
         ...
         do itr = 1, size(trans, 2)
            ...
            do ish = 1, bas%nsh_id(izp)
               ...
               do jsh = 1, bas%nsh_id(jzp)
                  ...
                  call nativeint%dipole_cgto(bas%cgto(jsh, jzp), &
                     & bas%cgto(ish, izp), js+jsh, is+ish, r2, vec, bas%intcut, &
                     & stmp, dtmp)
...
subroutine get_dipole_integrals_diat_lat(mol, trans, cutoff, bas, &
   ...
   !$omp parallel do schedule(runtime) default(none) &
   !$omp shared(mol, bas, trans, cutoff2, ksig, kpi, kdel, overlap, &
   !$omp& overlap_diat, dpint, nativeint) &
...
subroutine get_multipole_integrals_lat(mol, trans, cutoff, bas, overlap, dpint, qpint)
   ...
   !$omp parallel do schedule(runtime) default(none) &
   !$omp shared(mol, bas, trans, cutoff2, overlap, dpint, qpint, nativeint) &
   !$omp private(iat, jat, izp, jzp, itr, is, js, ish, jsh, ii, jj, iao, jao, nao) &
      ...
      do jat = 1, mol%nat
         ...
         do itr = 1, size(trans, 2)
            ...
            do ish = 1, bas%nsh_id(izp)
               ...
               do jsh = 1, bas%nsh_id(jzp)
                  jj = bas%iao_sh(js+jsh)
                  call nativeint%multipole_cgto(bas%cgto(jsh, jzp), &
                     & bas%cgto(ish, izp), js+jsh, is+ish, r2, vec, bas%intcut, &
                     & stmp, dtmp, qtmp)
...
!> Evaluate multipole integrals and diatomic frame scaled overlap
subroutine get_multipole_integrals_diat_lat(mol, trans, cutoff, bas, &
   ...
   !$omp parallel do schedule(runtime) default(none) &
   !$omp shared(mol, bas, trans, cutoff2, ksig, kpi, kdel, overlap, &
   !$omp& overlap_diat, dpint, qpint, nativeint) &
   !$omp private(iat, jat, izp, jzp, itr, is, js, ish, jsh, nsi, nsj) &
   !$omp private(ii, jj, ij, iao, jao, iaosh, jaosh, nao, r2, vec) &
