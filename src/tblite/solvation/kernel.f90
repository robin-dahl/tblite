...
module tblite_solvation_kernel
   ...
   public :: compute_kernel_dKdr_ij, compute_kernel_d2Kdr2_ij, compute_kernel_d3Kdr3_ij, compute_kernel_d4Kdr4_ij 
   ...
   contains
      ...
      procedure(compute_kernel_d4Kdr4_ij_interface), deferred :: compute_kernel_d4Kdr4_ij
   ...
   abstract interface
      ...
      subroutine compute_kernel_d4Kdr4_ij_interface(self, nat, xyz, brad, brdr, brdr2, brdr3, brdr4, i, j, d4Kdr4_ij)
         import :: kernel_type, wp
         class(kernel_type), intent(in) :: self
         integer, intent(in) :: nat
         real(wp), intent(in) :: xyz(:, :)
         ...
         real(wp), contiguous, intent(in) :: brdr2(:, :, :, :, :)
         real(wp), contiguous, intent(in) :: brdr3(:, :, :, :, :, :, :)
         real(wp), contiguous, intent(in) :: brdr4(:, :, :, :, :, :, :, :, :)
         integer, intent(in) :: i, j
         real(wp), contiguous, intent(out) :: d4Kdr4_ij(:, :, :, :, :, :, :, :)
      end subroutine compute_kernel_d4Kdr4_ij_interface
   ...
   contains
      ...
      procedure :: compute_kernel_d4Kdr4_ij => compute_still_d4Kdr4_ij
   ...
   contains
      ...
      procedure :: compute_kernel_d4Kdr4_ij => compute_p16_d4Kdr4_ij
   end type p16_kernel
   ...
   contains
      ...
      procedure :: compute_kernel_dKdr_ij => compute_coulomb_dKdr_ij
      procedure :: compute_kernel_d2Kdr2_ij => compute_coulomb_d2Kdr2_ij
      procedure :: compute_kernel_d3Kdr3_ij => compute_coulomb_d3Kdr3_ij
      procedure :: compute_kernel_d4Kdr4_ij => compute_coulomb_d4Kdr4_ij
   end type
...
subroutine compute_kernel_d4Kdr4_ij(kernel_id, keps, nat, xyz, brad, brdr, brdr2, brdr3, brdr4, i, j, d4Kdr4_ij)
   integer, intent(in) :: kernel_id
   real(wp), intent(in) :: keps
   integer, intent(in) :: nat
   real(wp), intent(in) :: xyz(:, :)
   ...
   kernel = new_kernel(kernel_id, keps)
   call kernel%compute_kernel_d4Kdr4_ij(nat, xyz, brad, brdr, brdr2, brdr3, brdr4, i, j, d4Kdr4_ij)
end subroutine compute_kernel_d4Kdr4_ij
...
subroutine compute_still_d4Kdr4_ij(self, nat, xyz, brad, brdr, brdr2, brdr3, brdr4, i, j, d4Kdr4_ij)
   class(still_kernel), intent(in) :: self
   integer, intent(in) :: nat
   real(wp), intent(in) :: xyz(:, :)                                 ! (3,nat)
   real(wp), intent(in) :: brad(:)                                   ! (nat)
   ...
   real(wp), contiguous, intent(in) :: brdr2(:, :, :, :, :)          ! (3,nat,3,nat,nat)
   real(wp), contiguous, intent(in) :: brdr3(:, :, :, :, :, :, :)    ! (3,nat,3,nat,3,nat,nat)
   real(wp), contiguous, intent(in) :: brdr4(:, :, :, :, :, :, :, :, :) ! (3,nat,3,nat,3,nat,3,nat,nat)
   integer, intent(in) :: i, j
   real(wp), contiguous, intent(out) :: d4Kdr4_ij(:, :, :, :, :, :, :, :) ! (3,nat,3,nat,3,nat,3,nat)
                     ...
                     do beta = 1,3
                        ...
                        do gamma = 1,3
                           ...
                           do delta = 1,3
                              ...
                              d4Kdr4_ij(alpha,k,beta,l,gamma,m,delta,n) = d4Kdr4_ij(alpha,k,beta,l,gamma,m,delta,n) + term
                           end do
                        end do
                     end do
                  end do
   ...
   ! compute always in the ip>jp ordering, store that value for (i,j).
   ! -------------------------
   if (i > j) then
      ip = i
      jp = j
               ...
               do alpha = 1,3
                  ...
                  do beta = 1,3
                     ...
                     do gamma = 1,3
                        ...
                        do delta = 1,3
                           dai_n = brdr(delta,n,ip)
                           daj_n = brdr(delta,n,jp)

                           d2ai_kn = brdr2(alpha,k,delta,n,ip)
                           d2ai_ln = brdr2(beta,l,delta,n,ip)
                           d2ai_mn = brdr2(gamma,m,delta,n,ip)

                           d2aj_kn = brdr2(alpha,k,delta,n,jp)
                           d2aj_ln = brdr2(beta,l,delta,n,jp)
                           d2aj_mn = brdr2(gamma,m,delta,n,jp)

                           d3ai_kln = brdr3(alpha,k,beta,l,delta,n,ip)
                           d3ai_kmn = brdr3(alpha,k,gamma,m,delta,n,ip)
                           d3ai_lmn = brdr3(beta,l,gamma,m,delta,n,ip)

                           d3aj_kln = brdr3(alpha,k,beta,l,delta,n,jp)
                           d3aj_kmn = brdr3(alpha,k,gamma,m,delta,n,jp)
                           d3aj_lmn = brdr3(beta,l,gamma,m,delta,n,jp)
                           ...
                           ! =========================================================
                           ! Build A-derivatives for this (k,l,m,n; alpha,beta,gamma,delta)
                           ! =========================================================
...
end subroutine compute_still_d4Kdr4_ij
...
subroutine compute_p16_d4Kdr4_ij(self, nat, xyz, brad, brdr, brdr2, brdr3, brdr4, i, j, d4Kdr4_ij)
   class(p16_kernel), intent(in) :: self
   integer, intent(in) :: nat
   real(wp), intent(in) :: xyz(:, :)                                 ! (3,nat)
   real(wp), intent(in) :: brad(:)                                   ! (nat)
   ...
   c = zetaP16o16   ! = zeta/16  (same as your d3)
...
end subroutine compute_p16_d4Kdr4_ij
...
subroutine compute_coulomb_dKdr_ij(self, nat, xyz, brad, brdr, i, j, dKdr_ij)
   class(coulomb_kernel), intent(in) :: self
   integer, intent(in) :: nat
   real(wp), intent(in) :: xyz(:, :)                 ! (3,nat)
   real(wp), intent(in) :: brad(:)                   ! (nat)
   ...
   if (i == j) then
      ! Diagonal: classic Coulomb kernel has no self term, so K_ii = 0  =>  dK_ii/dr_k = 0
      return
   end if

   ! Off-diagonal element (i,j): K_ij = keps / |r_i - r_j|
   rvec(:) = xyz(:, i) - xyz(:, j)
   r2      = dot_product(rvec, rvec)
   r1      = sqrt(r2)
   ...
   ! dK_ij/dr_i = -keps * (r_i - r_j) / r^3
   ! dK_ij/dr_j = +keps * (r_i - r_j) / r^3
   ...
   ! No chain rule via Born radii for pure Coulomb kernel:
   ! brad and brdr are kept only for interface compatibility.
end subroutine compute_coulomb_dKdr_ij

subroutine compute_coulomb_d2Kdr2_ij(self, nat, xyz, brad, brdr, brdr2, i, j, d2Kdr2_ij)
   class(coulomb_kernel), intent(in) :: self
   integer, intent(in) :: nat
   real(wp), intent(in) :: xyz(:, :)                        ! (3,nat)
   real(wp), intent(in) :: brad(:)                          ! (nat)
   ...
   real(wp) :: I3(3,3), vv(3,3), H(3,3), M(3,3)
   ...
   d2Kdr2_ij(:, :, :, :) = 0.0_wp
   ! -------------------------
   ! Diagonal self terms: classic Coulomb kernel has no self term
   ! K_ii = 0  =>  d2K_ii / dr_k dr_l = 0
   ! -------------------------
   if (i == j) then
      return
   end if
   ...
   ! Hessian w.r.t. v-components (i.e. x = r_i - r_j):
   ! d^2(1/r)/dx_a dx_b = (3 v_a v_b - r^2 delta_ab) / r^5
   ! H = (self%keps / r5) * (3.0_wp*vv - r2*I3)
   H = (1.0_wp / r5) * (3.0_wp*vv - r2*I3)
   ...
   do k = 1, nat
      ...
      do l = 1, nat
         ...
         ! Since K depends only on v = r_i - r_j,
         ! d/d r_k = delk * d/d v, so d2/d r_k d r_l = delk*dell * d2/d v^2.
         if (delk /= 0 .and. dell /= 0) then
            M = M + real(delk*dell, wp) * H
         end if
   ...
   ! brad, brdr, brdr2 kept only for interface compatibility.
end subroutine compute_coulomb_d2Kdr2_ij
...
subroutine compute_coulomb_d4Kdr4_ij(self, nat, xyz, brad, brdr, brdr2, brdr3, brdr4, i, j, d4Kdr4_ij)
   class(coulomb_kernel), intent(in) :: self
   integer, intent(in) :: nat
   real(wp), intent(in) :: xyz(:, :)                                     ! (3,nat)
   real(wp), intent(in) :: brad(:)                                       ! (nat)  (unused)
   real(wp), contiguous, intent(in) :: brdr(:, :, :)                     ! (3,nat,nat) (unused)
   real(wp), contiguous, intent(in) :: brdr2(:, :, :, :, :)              ! (3,nat,3,nat,nat) (unused)
   real(wp), contiguous, intent(in) :: brdr3(:, :, :, :, :, :, :)        ! (3,nat,3,nat,3,nat,nat) (unused)
   real(wp), contiguous, intent(in) :: brdr4(:, :, :, :, :, :, :, :, :)  ! (3,nat,3,nat,3,nat,3,nat,nat) (unused)
   integer, intent(in) :: i, j
   real(wp), contiguous, intent(out) :: d4Kdr4_ij(:, :, :, :, :, :, :, :) ! (3,nat,3,nat,3,nat,3,nat)
   ...
   ! Coulomb kernel has no self term: K_ii = 0 => all derivatives are zero
   if (i == j) return

   ! Match *_full semantics: always evaluate in the ip>jp ordering
   if (i > j) then
      ip = i
      jp = j
   else
   ...
   ! Build the 4th derivative tensor w.r.t. v-components:
   !
   ! ∂_a∂_b∂_c∂_d (1/r) =
   !   (105 v_a v_b v_c v_d
   !    - 15 r^2 * sym(δ_ab v_c v_d)
   !    +  3 r^4 * sym(δ_ab δ_cd)) / r^9
   !
   do alpha = 1,3
      do beta = 1,3
         do gamma = 1,3
            do delta = 1,3
               ...
               s1 = I3(alpha,beta)*v(gamma)*v(delta) &
                  + I3(alpha,gamma)*v(beta)*v(delta) &
                  + I3(alpha,delta)*v(beta)*v(gamma) &
                  + I3(beta,gamma)*v(alpha)*v(delta) &
                  + I3(beta,delta)*v(alpha)*v(gamma) &
                  + I3(gamma,delta)*v(alpha)*v(beta)

               s2 = I3(alpha,beta)*I3(gamma,delta) &
                  + I3(alpha,gamma)*I3(beta,delta) &
                  + I3(alpha,delta)*I3(beta,gamma)

               T4(alpha,beta,gamma,delta) = keps * ( &
                    105.0_wp * v(alpha)*v(beta)*v(gamma)*v(delta) &
                  -  15.0_wp * r2 * s1 &
                  +   3.0_wp * r4 * s2 ) / r9
            end do
         end do
   ...
   ! Map v-derivatives to coordinate derivatives using del-factors:
   ! d/d r_k = delk * d/d v,  where delk=+1 for k=ip, -1 for k=jp, else 0
   do k = 1, nat
      delk = 0; if (k==ip) delk=delk+1; if (k==jp) delk=delk-1
      if (delk == 0) cycle
            ...
            do n = 1, nat
               ...
               do alpha = 1,3
                  do beta = 1,3
                     do gamma = 1,3
                        do delta = 1,3
                           d4Kdr4_ij(alpha,k,beta,l,gamma,m,delta,n) = d4Kdr4_ij(alpha,k,beta,l,gamma,m,delta,n) &
                              + real(delk*dell*delm*deln, wp) * T4(alpha,beta,gamma,delta)
                        end do
                     end do
                  end do
               end do
...
end subroutine compute_coulomb_d4Kdr4_ij
