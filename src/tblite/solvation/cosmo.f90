...
subroutine update(self, mol, cache)
   ...
   call new_cavity_drop(ptr%cavity, verbose=0, nleb=self%nang, radius_model=radii, &
      ...
      do iat = 1, mol%nat
         do igrid = 1, ptr%cavity%ngrid
            ...
            if (r3 > epsilon(1.0_wp)) then
               ...
               ptr%ucore(igrid, iat) = erf(ptr%cavity%xi0(igrid)*r1)/r1
            else
               ! Analytic r -> 0 limit of erf(xi*r)/r.
               ptr%ucore(igrid, iat) = 2.0_wp*ptr%cavity%xi0(igrid)/sqrt(acos(-1.0_wp))
            end if
         end do
      end do
      call build_cosmo_density_integrals(mol, self%basis, ptr%cavity%xyz, &
         & ptr%cavity%xi0, ptr%bmat, error)
      if (allocated(error)) return
   end if
